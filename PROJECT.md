...
В bot.py настроен monkey-patch на `Message.answer` и `Message.reply`:
- **Все ответы бота в группах** удаляются через **15 секунд**
- **Медиа** (аудио, видео, фото, стикеры) **не удаляются** — пользователь должен успеть скачать
- **В личке** — ничего не удаляется
- **Приветствие** новых участников — висит **45 секунд** (отдельная логика в antispam.py)
